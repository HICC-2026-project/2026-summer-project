 ...
 *
 * 사용자별 상한은 AiDailyAttemptLimiter(하루 3회 시도)가 맡고, 이 가드는 그 위의 서비스 전체 상한이다 —
 * 사용자 수가 늘거나 키가 유출돼 여러 계정이 동시에 두드려도 하루 비용이 여기서 멈춘다. 성공·실패와 무관하게
 * "시도"를 세고, 상한에 닿으면 호출 자체를 건너뛰어 폴백으로 보낸다.
 *
