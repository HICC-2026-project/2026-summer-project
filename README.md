...
- 익명 합격자 분포 내 축별 percentile 위치·갭 계산 (v9 — 가중 총점 없음)
- 추천 활동마다 "어떤 갭을 메우는지"(targetGap) 표시, 비교 탭과 같은 갭 이름 사용
- 합격자 스펙 제보(증빙 이미지) → 관리자 검수(승인/반려) → 즉시 비교 데이터 반영
- 직무 표본 부족 시 제보 유도, 내 제보 검수 상태 확인
- 관리자 운영 요약(검수 대기·직무별 표본·Gemini 사용량), Gemini 일일 호출 상한(전역·사용자별)
- 활동 검색 필터(유형·직무·마감일·키워드), 마감 지난 활동 자동 비활성화
- 닉네임 변경, 회원 탈퇴, 리프레시 토큰 재사용 감지
