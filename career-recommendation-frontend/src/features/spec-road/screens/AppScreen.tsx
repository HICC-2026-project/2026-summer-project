...
export function AppScreen({ tab, onTabChange, spec, target, onOpenDetail, onEditSpec, onLogout }: AppScreenProps) {
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", background: "#F6F6F9" }}>
      <div className="cf-scroll" style={{ flex: 1, overflowY: "auto", overflowX: "hidden" }}>
        {tab === "home" && <HomeTab target={target} onOpenDetail={onOpenDetail} />}
        {tab === "roadmap" && <RoadmapTab target={target} />}
        {tab === "compare" && <CompareTab />}
        {tab === "profile" && <ProfileTab spec={spec} target={target} onEditSpec={onEditSpec} onLogout={onLogout} />}
      </div>
